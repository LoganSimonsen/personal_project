...
class DoughnutChart extends Component {
        ...
        return (
          ...
          onChange={isFull => this.setState({isFull})}>
            ...
            {this.props.user.id > 0 && <div className='chart' style={{backgroundColor: this.state.theme, marginTop: this.state.marginTop}}>
                    <Doughnut className='DoughnutChartInner' data={this.state.chartData2} options={chartOptions}/>
            </div>}
            {window.location.href.includes("DoughnutChart") && this.state.isFull === false && <button id='fullscreenButton' onClick={this.goFull}>
            Go Fullscreen
          </button>}
          {window.location.href.includes("DoughnutChart") && this.state.isFull === true && <button className='exitButton' id='fullscreenButton' onClick={this.exitFull}>
          Exit Fullscreen
        </button>}
        {window.location.href.includes("DoughnutChart") && <button id='toDarkThemeButton' onClick={this.changeTheme}>
         Theme
        </button>}
            </div>
            </Fullscreen>
    ...
    export default withRouter(connect(mapStateToProps, { getUser, logout})(DoughnutChart));